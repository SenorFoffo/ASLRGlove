	...
	*magConfig = MAG_DATA_OUTPUT_RATE_30HZ | MAG_OPR_MODE_HIGH_ACCURACY | MAG_POWER_MODE_NORMAL;
	...
	while (1)
	{
		...
		feedbackcounter++;*/
			...
			//for recording training data uncomment the following line, and comment the previous line.
		...
		/*switch(predictedClass)
		{
			case 2:
				...
				predictedCharacter = 0x61;

				if(!compareCharacters(predictedCharacter,lastPredictedCharacter))
				{
					printf("%c", predictedCharacter);
					memset(&lastPredictedCharacter, 0x61, sizeof(u8));
				}
				break;
			case 3:
				predictedCharacter = 0x62;
				if(!compareCharacters(predictedCharacter,lastPredictedCharacter))
				{
					printf("%c", predictedCharacter);
					memset(&lastPredictedCharacter, 0x62, sizeof(u8));
				}
				break;
			case 4:
				predictedCharacter = 0x64;
				if(!compareCharacters(predictedCharacter,lastPredictedCharacter))
				{
					printf("%c", predictedCharacter);
					memset(&lastPredictedCharacter, 0x64, sizeof(u8));
				}
				break;
			case 5:
				predictedCharacter = 0x69;
				if(!compareCharacters(predictedCharacter,lastPredictedCharacter))
				{
					printf("%c", predictedCharacter);
					memset(&lastPredictedCharacter, 0x69, sizeof(u8));
				}
				break;
			case 6:
				predictedCharacter = 0x6C;
				if(!compareCharacters(predictedCharacter,lastPredictedCharacter))
				{
					printf("%c", predictedCharacter);
					memset(&lastPredictedCharacter, 0x6C, sizeof(u8));
				}
				break;
			case 0:
				predictedCharacter = 0x3F;
				if(!compareCharacters(predictedCharacter,lastPredictedCharacter))
				{
					printf("%c", predictedCharacter);
					memset(&lastPredictedCharacter, 0x3F, sizeof(u8));
				}
				break;
			default:
				break;
